...
한국수출입은행 Open API의 미국 달러 매매기준율을 Supabase에 일별로 기록하고, 바로 어제의 값과 비교하는 정보판입니다. 흰색·파란색 반응형 화면, 7일/30일 차트, 일별 기록, 데이터 검증 영역을 제공합니다.
...
- 표시값은 **매매기준율 `deal_bas_r`**, 단위는 **원 / 1 USD**입니다. 쉼표와 소수점 두 자리 표시는 숫자의 의미를 바꾸지 않습니다.
